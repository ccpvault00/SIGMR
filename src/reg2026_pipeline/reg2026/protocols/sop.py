...
Pipeline (all organs):
  ...
  answering  : visual recognizer - slide-level ABMIL #1 dx + per-Q M1 answerers
               (cls head / MedCPT-cosine, standalone or context-aware)
...
PROSTATE = """PROSTATE - acinar adenocarcinoma (CAP + WHO)
...
BREAST = """BREAST - invasive carcinoma NST / DCIS (CAP + WHO)
...
COLON = """COLON - adenocarcinoma / tubulovillous adenoma (CAP + WHO)
  abnormality / #1 diagnosis : visual recognizer
...
STOMACH = """STOMACH - adenocarcinoma / dysplasia / NET (CAP + WHO)
  abnormality / #1 diagnosis : visual recognizer
...
BLADDER = """BLADDER - urothelial carcinoma / CIS (CAP + WHO)
  abnormality / #1 diagnosis : visual recognizer
...
LUNG = """LUNG - non-small-cell carcinoma: adeno / squamous (CAP + WHO)
  abnormality / #1 diagnosis : visual recognizer
  histologic type            : visual (adenocarcinoma vs squamous cell carcinoma)
  favor / definitive         : reporting-convention hedge
"""

CERVIX = """CERVIX - squamous cell carcinoma / HSIL-LSIL (CAP + WHO)
  abnormality / #1 diagnosis : visual recognizer
  histologic type            : visual (M1)
