...
# Interfaces to run. Default = platform's two interfaces. Override for targeted local
# tests, e.g. INTERFACES=interf1_monster sudo bash do_test_run.sh (fast-path monster check).
INTERFACES="${INTERFACES:-interf0 interf1}"
for iface in $INTERFACES; do
    mkdir -p -m o+rwX "${OUTPUT_DIR}/${iface}"
    run_docker_forward_pass "$iface"
done
